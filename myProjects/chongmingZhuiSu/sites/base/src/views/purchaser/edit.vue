<template>
  <cjm-form ref="form" :model="form" :rules="rules">
    <cjm-form-item label="采购商名称" prop="name" required>
      <cjm-input v-model="form.name"></cjm-input>
    </cjm-form-item>
    <cjm-form-item label="采购商编号" prop="code">
      <cjm-input v-model="form.code"></cjm-input>
    </cjm-form-item>
    <cjm-form-item label="采购商类型" prop="type" required>
      <cjm-enum-select v-model="form.type" enum-name="采购商类型" enum-namespace="base"></cjm-enum-select>
    </cjm-form-item>
  ...
  methods: {
    ...
    submit() {
      return this.$refs.form.validate().then(() => {
        this.$ajax.post(this.$globalData.get('base', 'apiUrl').purchaser.EDIT, this.form).then(r => {
          if (r.resultCode != '200') {
