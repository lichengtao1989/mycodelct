<template>
  <div>
    <div v-if="currentPage == 'list'">
      <cjm-list :request-url="requestUrl" :need-base-search="true" :need-advanced-search="true" base-search-placeholder="请输入兑换名称">
        <div slot="operate">
        ...
        <div slot="search">
          ...
          <cjm-search-item label="兑换券名称" search-key="voucherName" v-model="search.voucherName">
        ...
        <template slot="tableColumns">
          <cjm-table-column sortable="custom" prop="voucherName" label="兑换券名称" min-width="150"></cjm-table-column>
...
<script>
import exchangeEdit from './exchange-edit.vue';
...
export default {
  ...
  data() {
    return {
      requestUrl: this.$globalData.get('marketing', 'apiUrl').activity.voucher.EXCHANGE_LIST,
      search: {
        dateRange: '',
        voucherName: '',
        productID: '',
