<template>
  <transition name="nz-stack-card-wrap">
    <!--卡片包裹容器-->
    <div class="nz-stack-card-wrap nz-card-wrap small" v-if="visible">
      <transition-group name="nz-stack-card"
                        tag="div"
                        class="nz-stack-card-container"
                        @before-leave="beforeLeave"
                        @before-enter="beforeEnter">
        <!--单个卡片-->
        <nz-card-inside v-for="(item, index) in showList"
                         :key="item.id"
                         :title="item.title"
          ...
          <template slot="bottomLeftOperate">
            <div class="nz-card-footer-pagination">
              <i class="nz-icon-chevron-down nz-icon-rotate-90" @click="showPreCard"></i>
              <a class="nz-card-footer-pagination-current">{{getIndex(item)}}</a>
              <a class="nz-card-footer-pagination-split">/</a>
              <a class="nz-card-footer-pagination-total">{{total}}</a>
              <i class="nz-icon-chevron-down nz-icon-rotate-n90" @click="showNextCard"></i>
            </div>
          </template>
...
<style rel='stylesheet/less' lang='less'>
  ...
  .nz-stack-card-wrap-leave-active {
    transform: scale(0, 0);
  ...
  .nz-card-footer-pagination-current{
    color: @primaryColor;
    margin-right: 2px;
  }
  .nz-card-footer-pagination-split{
    margin-right: 2px;
  }
 </style>
