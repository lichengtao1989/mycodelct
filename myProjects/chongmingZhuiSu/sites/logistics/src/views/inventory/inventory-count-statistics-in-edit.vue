<template>
  <cjm-form ref="form" :model="form" :rules="rules">
    ...
    <cjm-form-item label="入库数量">
      {{form.inNum}}
    </cjm-form-item>
    <cjm-table :data="tableData">
      <cjm-table-column prop="type" label="入库类型" enum-namespace="logistics" enum-name="台账出入库科目"></cjm-table-column>
      <cjm-table-column prop="number" label="入库数量"></cjm-table-column>
    </cjm-table>
  </cjm-form>
</template>
<script>
  export default{
    ...
    methods: {
      ...
      editForm(orgID, productID, stockTime) {
        this.reset();
        this.$ajax.get(this.$globalData.get('logistics', 'apiUrl').inventory.GET_LEDGER_DETAIL, {
          orgID: orgID,
          productID: productID,
          stockTime: stockTime
        }).then((r) => {
