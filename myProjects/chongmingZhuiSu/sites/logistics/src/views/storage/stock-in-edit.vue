<template>
  <cjm-form :model="form" :rules="rules" ref="form">
    ...
    <cjm-form-item class="cjm-form-item-long" label="库房名称" prop="stockInHouseID" :required="isStockInHouseRequire">
      <cjm-remote-select :remote-url="logisticsApiUrl.autoComplete.STOCK_HOUSE" v-model="form.stockInHouseID" :disabled="stockInHouseDisabled" :label="stockInHouseName" :params="{otherStockIn:1}"></cjm-remote-select>
    </cjm-form-item>
    <cjm-form-item class="cjm-form-item-long" label="入库方式" prop="codeBindType" required>
      <cjm-radio-group v-model="form.codeBindType">
        <cjm-radio key="1" label="1">按号段</cjm-radio>
        <cjm-radio key="3" label="3">单码</cjm-radio>
      </cjm-radio-group>
    </cjm-form-item>
    <div class="codeArea clear">
      <cjm-form-item key="beginLogisticsCode" label="起始码" prop="beginLogisticsCode" required v-if="form.codeBindType==1" class="cjm-form-item-long">
        <cjm-input v-model="form.beginLogisticsCode">
        </cjm-input>
      </cjm-form-item>
...
export default {
  data() {
    ...
    let _orgType = this.$globalData.get('currentUser').orgType; //机构类型
    let _corpType = this.$globalData.get('currentUser').corpType; //组织类型
    let orgTypeEnums = this.$globalData.get('logistics', 'enums').orgType;
    let corpTypeEnums = this.$globalData.get('logistics', 'enums').corpType;
    let isStockInHouseRequire = true;
    if (orgTypeEnums.COMPANY == _orgType && corpTypeEnums.GOVERNMENT == _corpType) {
      isStockInHouseRequire = false;
    }
    let logisticsApiUrl = this.$globalData.get('logistics', 'apiUrl');
    return {
      logisticsApiUrl: logisticsApiUrl,
      orgType: orgTypeEnums,
      corpType: corpTypeEnums,
      form: {},
      msgDetails: [],
      stockInHouseName: '',
      productCopyID: '',
      rules: {
        stockInHouseID: isStockInHouseRequire ? [this.$formRules.required] : [],
  ...
  computed: {
    isStockInHouseRequire() {
      let _orgType = this.$globalData.get('currentUser').orgType; //机构类型
      let _corpType = this.$globalData.get('currentUser').corpType; //组织类型
      if (this.orgType.COMPANY == _orgType && this.corpType.GOVERNMENT == _corpType) {
        return false;
      }
      return true;
    },
  ...
  methods: {
    ...
    reset() {
      ...
      if (this.$globalData.get('currentUser').orgType == this.orgType.STOCK_HOUSE) {
    ...
    submit() {
      ...
      return this.$refs.form.validate().then(() => {
        return this.$ajax.post(this.logisticsApiUrl.storage.STOCK_IN, this.form).then(r => {
          if (r.resultCode != '200') {
