<template>
  <div :key="$route.meta.code">
    <cjm-list :request-url="requestUrl" :need-advanced-search="true" ref="list"
      ...
      <div slot="search">
        <cjm-search-item :label="labelName" v-if="$route.meta.code == 'headStockIn'" search-key="houseName" v-model="search.houseName">
        ...
        <cjm-search-item label="科目" search-key="subjectID" v-model="search.subjectID">
          <cjm-enum-select v-model="search.subjectID" enum-namespace="logistics" enum-name="物流入库方式" :clearable="true"></cjm-enum-select>
        </cjm-search-item>
        <cjm-search-item label="入库时间" search-key="createTime" v-model="search.createTime">
  ...
  export default{
    data(){
      ...
      return {
        ...
        url: {
          'headStockIn': logisticsApiUrl.inventory.STOCK_LIST + '&ioFlag=1',
          'dealerStockIn': logisticsApiUrl.inventory.STOCK_STATS_LIST + '&ioFlag=1&orgType=' + orgType.DEALER,
          'storeStockIn': logisticsApiUrl.inventory.STOCK_STATS_LIST + '&ioFlag=1&orgType=' + orgType.SHOP
        },
        exportUrl: {
          'headStockIn': logisticsApiUrl.inventory.EXPORT_STOCK + '&ioFlag=1',
          'dealerStockIn': logisticsApiUrl.inventory.EXPORT_STOCK_STATS + '&ioFlag=1&orgType=' + orgType.DEALER,
          'storeStockIn': logisticsApiUrl.inventory.EXPORT_STOCK_STATS + '&ioFlag=1&orgType=' + orgType.SHOP
        }
      }
    },
    computed: {
      ...
      propName() {
        return (this.$route.meta.code == 'headStockIn') ? 'houseName' : 'orgName';
      },
      labelName(){
        return (this.$route.meta.code == 'headStockIn') ? '库房名称' : '机构名称';
      },
      powerCode(){
