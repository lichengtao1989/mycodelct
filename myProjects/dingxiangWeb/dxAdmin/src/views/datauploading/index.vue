<template>
  <div class="newsindexout" id="MATERIALlct">
    <nz-list :requestUrl="$apiUrl.MATERIAL.LIST" ref="list" :beforeSearch="beforeSearchFn" :needAdvancedSearch="false" :needBaseSearch="false">
      <div slot="operate">
      ...
      <template slot="tableColumns">
        ...
        <nz-table-column prop="CreateTime" min-width="120" label="创建时间">
...
export default {
  ...
  methods: {
    ...
    async deleteFn(row) {
      ...
      if (result) {
        const { err, res } = await this.$ajax.get(this.$apiUrl.MATERIAL.DELETE, { MaterialID: row.MaterialID });
        console.log(err, res);
        if (err) {
