<template>
  <div>
    <div v-for="(item,index) in menuData" :key="index" class="nz-menu-item">
      <div class="nz-menu-item-head" :class="isActive(item)?'active':''">
        ...
        <router-link class="rlink" @click.native="clearActiveItem" v-if="isLink(item)" :to="'/main/'+item.powerCode">
          ...
          <i class="menu-icon nz-icon-caret-down nz-icon-rotate-n90" v-else></i>
          <span>{{item.powerName}}</span>
        </router-link>
        <!--有子菜单-->
        <span class="rlink" @click="toggleItemActive(item)" v-if="isParent(item)">
          <i class="menu-icon nz-icon-caret-down" :class="{'nz-icon-rotate-n90': viewChildItem!==item}"></i>
          <span>{{item.powerName}}</span>
        </span>
...
export default {
  ...
  computed: {
    menuData() {
      return this.$globalData.get('menuData') || [];
    }
  },
  methods: {
    clearActiveItem() {
  ...
  async beforeCreate() {
    await this.$model('system').menu.syncMenuData();
    this.setDefaultViewChildItem();
  },
  created() {
    this.$eventBus.$on('toggleItemActive', this.toggleItemActive);
  }
};
</script>
