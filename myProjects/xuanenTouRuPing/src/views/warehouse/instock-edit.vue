  ...
  <div>
    <nz-form ref="form" label-width="110px" :model="form" :rules="rules">
      <nz-form-item label="商品分类" prop="category">
        <nz-enum-select v-model="form.category" :options="$enum('类别').values" placeholder="" disabled="true"></nz-enum-select>
      </nz-form-item>
      ...
      <nz-form-item label="商品数量" prop="value" required>
        <nz-input v-model="form.value" placeholder="请输入当前库存数量"></nz-input>
      </nz-form-item>
    </nz-form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async addForm(row) {
      ...
      this.form.value = row.stockCount;
    }
  },
  created() {
    this.reset();
