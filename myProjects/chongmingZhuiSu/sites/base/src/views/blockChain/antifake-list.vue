<template>
  <div>
    <div v-if="currentPage == 'list'">
      <cjm-list ref="list"
                :request-url="apiUrl.codeGeneration.GET_LIST"
        ...
        <div slot="search">
          <cjm-search-item label="组织" search-key="corpID" v-model="search.corpID">
            <cjm-remote-select v-model="search.corpID"
                               :remote-url="apiUrl.autoComplete.CORP">
              <template slot-scope="scope">
                {{scope.item.name}}<span class="cjm-option-subtitle">（{{scope.item.id}}）</span>
              </template>
            </cjm-remote-select>
          </cjm-search-item>
