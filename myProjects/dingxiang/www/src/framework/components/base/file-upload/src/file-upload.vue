<template>
  <div class="nz-image-upload">
    <template v-if="isShowItems">
      <image-upload-item v-for="(path,index) in val" :key="path+'-'+index" :uploadUrl="uploadUrl" :value="path" @uploadSuccess="editPath(index,$event)" @del-image="delImage(index)" @fileInfo="fileInfo"></image-upload-item>
      <image-upload-item v-if="val.length < maxNumber" :uploadUrl="uploadUrl" key="novalue" @uploadSuccess="addPath"></image-upload-item>
      <div class="nz-image-upload-tips" v-if="tips" @fileInfo="fileInfo">({{tips}})</div>
    </template>
  </div>
</template>
<script>
import imageUploadItem from './file-upload-item.vue';
export default {
  name: 'nz-file-upload',
