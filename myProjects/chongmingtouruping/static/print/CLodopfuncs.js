      ...
      if (oType !== undefined && oValue !== undefined) {
        if (oType.replace(/^\s+|\s+$/g, '').toLowerCase().indexOf('time:') == 0) {
          ...
          if (oType.toLowerCase() == 'isvalidformat') { oValue = (iYear > 0 && iMonth > 0 && iMonth <= 12 && iDate > 0 && iDate <= 31); } else {
            ...
            if (oType.toLowerCase() == 'weekindex') { oValue = iDay; } else
            if (oType.toLowerCase() == 'floatvalue') { oValue = dValue.getTime(); } else {
              var sWeek = '';
              switch (iDay) {
                case 0:
                  sWeek = '日'; break;
    ...
    GET_PRINTER_NAME: function(intNO) {
      if (this.Printers === undefined) { return ''; } else {
        if (typeof intNO == 'string' && intNO.indexOf(':') > -1) {
          ...
          if (intNO == -1) { return this.Printers['list'][this.Printers['default']].name; } else
          if (intNO >= 0 && intNO < this.Printers['list'].length) { return this.Printers['list'][intNO].name; } else { return 'Printer NO. overflow'; }
        }
      }
    },
