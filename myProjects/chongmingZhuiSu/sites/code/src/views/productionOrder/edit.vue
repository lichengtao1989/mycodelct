<template>
  <cjm-dialog ref="dialog" :visible.sync="dialogVisible" type="step" :height="850">
    ...
    <edit-base ref="formBase" :form="form" :status="status" :isApprove="isApprove" :operation="operation"
               @corp-changed="corpChanged" v-show="steps[activeStep].value===allSteps.base"></edit-base>
    <edit-antifake ref="formAntifake" :form="form" :status="status" :isApprove="isApprove" :operation="operation"
                   v-if="form.functionType.includes(enums.codeFunctionType.ANTIFAKE)"
                   v-show="steps[activeStep].value===allSteps.antifake"></edit-antifake>
    <edit-logistics ref="formLogistics" :form="form" :status="status" :isApprove="isApprove" :operation="operation"
                    :corp-logistics-layer-number="corpLogisticsLayerNumber"
                    v-if="form.functionType.includes(enums.codeFunctionType.LOGISTICS)"
                    v-show="steps[activeStep].value===allSteps.logistics"></edit-logistics>
    <edit-integral ref="formIntegral" :form="form" :status="status" :isApprove="isApprove" :operation="operation"
                   v-if="form.functionType.includes(enums.codeFunctionType.INTEGRAL)"
                   v-show="steps[activeStep].value===allSteps.integral"></edit-integral>
    <cjm-form ref="formApprove" v-if="operation==='approve'" v-show="steps[activeStep].value===allSteps.approve"
              :model="formForApprove" :rules="rulesForApprove">
      <cjm-form-item label="审核结果" prop="value" required>
        <cjm-radio-group v-model="formForApprove.value">
          <cjm-radio label="-2">审核通过</cjm-radio>
          <cjm-radio label="-3">审核不通过</cjm-radio>
        </cjm-radio-group>
      </cjm-form-item>
    </cjm-form>
    <div slot="footer">
      ...
      <cjm-button type="primary" @click="submit" v-if="operation!=='approve' && operation!=='view'"
                  v-show="activeStep+1==steps.length" :loading="loading">
        <i class="cjm-icon-confirm"></i>确定
      </cjm-button>
      <cjm-button type="primary" @click="approveSubmit" v-if="operation==='approve'" v-show="activeStep+1==steps.length"
                  :loading="loading">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //新增
      addForm(productionOrderID, corpID, orderCode, salesman, corpName) {
        this.reset();
        this.form.corpID = corpID;
        this.form.orderCode = orderCode;
        this.form.salesman = salesman;
        this.$nextTick(() => {
          this.form.userCorpName = corpName;
        });
        this.productionOrderID = productionOrderID;
        this.operation = 'add';
      },
      //编辑
      editForm(id, corpID, orderCode, salesman) {
        this.reset();
        this.form.corpID = corpID;
        this.form.orderCode = orderCode;
        this.form.salesman = salesman;
        ...
        this.$ajax
          ...
          .then(r => {
            if (r.resultCode === '200') {
              ...
              this.form.queryType = JSON.parse(this.form.queryType);
      ...
      //复制
      copyForm(id, productionOrderID, corpID, orderCode, salesman, corpName) {
        this.reset();
        this.form.corpID = corpID;
        this.form.orderCode = orderCode;
        this.form.salesman = salesman;
      ...
      submit() {
        this.validate()
          .then(() => {
            ...
            this.form.status = -5;
            if (!this.form.id) {
              this.form.productionOrderID = this.productionOrderID;
            }
            this.form.fileNameLabel = this.form.tagName;
            this.form.smsQueryText = this.form.internetQueryText;
            this.$ajax
      ...
      approveSubmit() {
        this.$refs.formApprove.validate(val => {
          if (val) {
            this.loading = true;
            this.$ajax
              .post(this.apiUrl.productionOrder.APPROVE, this.formForApprove)
              .then(r => {
                if (r.resultCode === '200') {
                  this.dialogVisible = false;
                  this.$emit('approveSuccess', this.formForApprove.value);
                } else {
                  this.$message.error(r.msg);
                }
                this.loading = false;
      ...
      reset() {
        ...
        this.form = {
          ...
          stickerImage: '',
          tagName: '', //标识名称
          orderBatchNumber: '', //订单数量
          tagSpecifications: '', //标签规格
          emergencyDegree: '1', //紧急程度 1正常 2加急
          isSupplement: '0', //是否补单
          queryType: [], //查询方式
          generatingNote: '', //生码备注
          printingNote: '', //印刷备注
          factoryID: '', //印刷工厂-ID
          factoryName: '', //印刷工厂-Name
          reqDeliveryTime: '',
          paperRequirements: '',
          isProductionOrder: '1',
          /************防伪信息***************/
