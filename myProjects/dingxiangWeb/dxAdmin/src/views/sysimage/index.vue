<template>
  <div class="newsindexout">
    <nz-list :requestUrl="$apiUrl.SYSIMAGE.LIST" ref="list" :beforeSearch="beforeSearchFn" :needAdvancedSearch="false" :needBaseSearch="false">
      <div slot="operate">
      ...
      <template slot="tableColumns">
        ...
        <nz-table-column prop="ImagePath" min-width="120" label="图片">
          <template slot-scope="scope">
            <img :src="scope.row.ImagePath" class="tpsm">
          </template>
        </nz-table-column>
        <nz-table-column prop="ImageName" min-width="120" label="说明">
        </nz-table-column>
...
export default {
  ...
  methods: {
    ...
    async deleteFn(row) {
      ...
      if (result) {
        const { err, res } = await this.$ajax.get(this.$apiUrl.SYSIMAGE.DELETE, { ImgID: row.ImgID });
        console.log(err, res);
        if (err) {
