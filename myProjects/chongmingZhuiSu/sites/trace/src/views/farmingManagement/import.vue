<template>
  <cjm-form ref="form" :model="form" :rules="rules">
    ...
    <cjm-form-item label="文件上传" required class="cjm-form-item-long">
      ...
      <cjm-upload ref="upload" class="upload-button" name="fileData" :action="apiUrl.poorFarmily.UPLOAD_EXCEL" accept=".xls,.xlsx" :auto-upload="true" :show-file-list="false" :on-success="uploadSuccess" :on-error="uploadError" :on-change="fileChange">
...
export default {
  ...
  data() {
    return {
      uploadUrl: this.$globalData.get('trace', 'apiUrl').FarmingManagement.IMPORT,
  ...
  methods: {
    ...
    submit() {
      return new Promise((resolve, reject) => {
        ...
        this.$ajax.post(this.$globalData.get('trace', 'apiUrl').FarmingManagement.IMPORT, { url }).then(res => {
          // this.$ajax.post(this.$globalData.get('trace', 'apiUrl').FarmingManagement.IMPORT).then(res => {
          this.fileName = '';
          this.filePath = '';
